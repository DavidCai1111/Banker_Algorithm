...
router.get('/', function(req, res) {
    ...
    lenOfP--;
    res.render('index', { title: '银行家算法',table:req.session.table,lenOfP:lenOfP});
});
...
router.post('/validate', function(req, res) {
    ...
    if(passed.length === len){
        res.render('index', { title: '银行家算法' ,succeed:true,passed:passed,requ:req.session.requ,table:req.session.table,lenOfP:len});
    }
    else{
        res.render('index', { title: '银行家算法' ,succeed:false,passed:passed,requ:req.session.requ,table:req.session.table,lenOfP:len});
    }
});
...
router.post('/sendReq', function(req, res) {
    ...
    if(parseInt(_requ[3]) >= len){
        return res.render('index', { title: '银行家算法' ,succeed:true,vali:vali,requ:req.session.requ,table:req.session.table,lenOfP:len});
    };
    ...
    if(passed.length === len){
        ...
        res.render('index', { title: '银行家算法' ,succeed:true,finish:true,vali:vali,passed:passed,requ:req.session.requ,table:req.session.table,lenOfP:len});
    }else{
        res.render('index', { title: '银行家算法' ,succeed:true,finish:false,vali:vali,passed:passed,requ:req.session.requ,table:req.session.table,lenOfP:len});
    }
});
